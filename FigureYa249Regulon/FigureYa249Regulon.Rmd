...
Many of the pictures in the article have been crowdsourced, such as:
...
easyinput_exp_phenotype.txt，每个样本对应的分组信息（Treat/Model）。

easyinput_regulon.txt，调整为脂质代谢常见的50种regulon基因。
...
exp.mtx.txt, the expression matrix。

easyinput_exp_phenotype.txt, the group information corresponding to each sample (Treat/Model).

easyinput_regulon.txt, adjusted to 50 commonly used lipid-metabolism-related regulon genes.
...
```{r}
# 加载基因表达以及样本分组信息
# Load gene expression and sample group information
hepg2_oa_expr <- read.table("exp.mtx.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)
hepg2_group <- read.table("easyinput_exp_phenotype.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)

# 加载MIBC特异性的调控子
# Load MIBC-specific regulators
lipid_regulon_tfs <- read.table("easyinput_regulon.txt",header = T)
```
...
Bootstrap calculates that the reference regulatory network takes a long time to run.

In order to facilitate the repeated running of the debugging code, the intermediate data is saved to a file. You can skip this step and go to Start Drawing.

```{r}
# 取共有的基因名
# Take the common gene name
regulatoryElements <- intersect(lipid_regulon_tfs$regulon, rownames(hepg2_oa_expr))

# 运行TNI构建程序
# Run the TNI builder
# we used the R package “RTN” to reconstruct transcriptional regulatory networks (regulons)
rtni_hepg2_oa <- tni.constructor(expData = as.matrix(log2(hepg2_oa_expr + 1)), # 样图计算时候没有取对数,  # There is no logarithm when calculating the sample,
...
# 小样本（例如本示例仅6个样本）可能出现没有可用于DPI三角过滤的边，导致warning；这里做保护性处理
# In small-sample datasets (e.g., 6 samples in this example), there may be no valid triangles for DPI filtering.
# We therefore apply a guarded call and fall back to the unfiltered network if DPI filtering cannot be applied.
min_regulon_size <- 5
dpi_filtered <- try(
  suppressWarnings(
    tni.dpi.filter(rtni_hepg2_oa, eps = 0, sizeThreshold = TRUE, minRegulonSize = min_regulon_size)
  ),
  silent = TRUE
)
if (!inherits(dpi_filtered, "try-error")) {
  rtni_hepg2_oa <- dpi_filtered
} else {
  message("DPI filter skipped: no valid edges/triangles for this dataset; continue with permutation/bootstrap-stable network.")
}

# 保存TNI对象以便后续分析
# Save the TNI object for later analysis
save(rtni_hepg2_oa, file="rtni_hepg2_oa.RData")

# load("rtni_hepg2_oa.RData")
# 计算每个样本的regulon活性
# Calculate the regulon activity for each sample
# Individual regulon activity was estimated by two-sided GSEA
rtnigsea_hepg2_oa <- try(
  tni.gsea2(rtni_hepg2_oa, regulatoryElements = regulatoryElements, minRegulonSize = min_regulon_size),
  silent = TRUE
)
if (inherits(rtnigsea_hepg2_oa, "try-error")) {
  message("GSEA retry with minRegulonSize=1 for small-sample dataset...")
  rtnigsea_hepg2_oa <- try(
    tni.gsea2(rtni_hepg2_oa, regulatoryElements = regulatoryElements, minRegulonSize = 1),
    silent = TRUE
  )
}
if (inherits(rtnigsea_hepg2_oa, "try-error")) {
  stop("tni.gsea2 failed after fallback. Please inspect regulon network size or increase sample size.")
}
hepg2_regact <- tni.get(rtnigsea_hepg2_oa, what = "regulonActivity")

# 保存活性对象
# Save the active object
save(hepg2_regact,file = "hepg2_regact.RData") 
```
...
(load("hepg2_regact.RData"))

# 设置颜色
# Set colors according to groups
group_levels <- unique(hepg2_group$Group)
palette_base <- c("#DD492E","#40548A","#32A087","#EC7D21","#7B6FD0","#2A9D8F")
clust.col <- setNames(palette_base[seq_along(group_levels)], group_levels)
blue <- "#5bc0eb"
gold <- "#ECE700"

plotdata <- standarize.fun(t(hepg2_regact$differential),halfwidth = 1.5) # 标准化regulon的活性 # Normalize the activity of regulon 
annCol.hepg2 <- hepg2_group[order(hepg2_group$Group),,drop = F] # 构建样本注释信息，并对分组进行排序 # Construct sample annotation information and sort groups
annColors.hepg2 <- list()
annColors.hepg2[["Group"]] <- clust.col
hcg <- hclust(distanceMatrix(as.matrix(hepg2_regact$differential[rownames(annCol.hepg2),]), "euclidean"), "ward.D")
group_counts <- cumsum(table(annCol.hepg2$Group))
gap_positions <- if (length(group_counts) > 1) group_counts[-length(group_counts)] else NULL
hm <- pheatmap(plotdata[hcg$order,rownames(annCol.hepg2)],
               ...
               gaps_col = gap_positions, # 分组分割 # Group segmentation
               cellwidth = 0.8, # 固定单元格宽度 # Fixed cell width
               cellheight = 10, # 固定单元格高度 # Fixed cell height
               name = "HepG2 Regulon", # 图例名字 # Legend name
               annotation_col = annCol.hepg2[,"Group",drop = F], # 样本注释 # Sample annotations
               annotation_colors = annColors.hepg2["Group"]) # 样本注释的对应颜色 # The color of the sample annotation
