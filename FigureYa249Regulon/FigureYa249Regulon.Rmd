...
tcgaBLCA <- read.table("exp.mtx.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)
pheno <- read.table("easyinput_exp_phenotype.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)
...
group_levels <- unique(pheno$Group)
...
annCol.tcga <- pheno[order(pheno$Group),,drop = F] # 构建样本注释信息，并对分组进行排序 # Construct sample annotation information and sort groups
annColors.tcga <- list()
annColors.tcga[["Group"]] <- clust.col
hcg <- hclust(distanceMatrix(as.matrix(MIBC_regact$differential[rownames(annCol.tcga),]), "euclidean"), "ward.D")
group_counts <- cumsum(table(annCol.tcga$Group))
...
hm <- pheatmap(plotdata[hcg$order,rownames(annCol.tcga)],
               ...
               annotation_col = annCol.tcga[,"Group",drop = F], # 样本注释 # Sample annotations
               annotation_colors = annColors.tcga["Group"]) # 样本注释的对应颜色 # The color of the sample annotation
