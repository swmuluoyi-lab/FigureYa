...
params:
  author: "Dongqiang Zeng, Ying Ge"
  reviewer: "Yijing Chen"
output: 
  html_document:
    toc: true
    toc_depth: 3
...
c森林图的画法可参考FigureYa90subgroup来画，h和i图可参考FigureYa33DCA_update。
...
library(regplot) # 新型nomogram new nomogram
library(rms) # 经典nomogram classic nomogram
library(nomogramEx) # 输出nomogram公式 output nomogram formula
Sys.setenv(LANGUAGE = "en") #显示英文报错信息 display English error messages
options(stringsAsFactors = FALSE) #禁止chr转成factor prohibit the conversion of chr to factor

# 加载修改后的 regplot 函数，使其支持非交互式环境生成 PDF
# Load modified regplot function to support non-interactive PDF generation
source("regplot_functions_modified.R")
source("regplot_modified.R")
# 如果需要使用原版 regplot，请注释掉上面两行代码
# If you need to use the original regplot, please comment out the above two lines of
```
...
## 画新型nomogram Draw new nomogram

`regplot`可以交互式调整nomogram，代码更简单，输出的图形更漂亮。

本模块已加载修改后的 `regplot` 函数，使其能够在渲染过程中自动生成 PDF 文件。

`regplot` can interactively adjust the nomogram, the code is simpler, the output graph is more beautiful.

This module loads a modified `regplot` function that can automatically generate PDF files during rendering.

### Cox 回归 nomogram Cox Regression Nomogram

```{r, message=FALSE, warning=FALSE}
pbccox <- coxph(formula = Surv(time,died) ~  age + catbili + sex +
                   copper + stage + trt , data = pbc)

# 生成 Cox 回归 nomogram PDF
# Generate Cox regression nomogram PDF
pdf("nomogram_regplot_cox.pdf", width = 12, height = 8)
regplot(pbccox,
        ...
        subticks = TRUE,
        clickable = FALSE, # 非交互式环境必须设置为 FALSE / Must be FALSE in non-interactive environment
...
![](nomogram_regplot_cox.pdf)

### 逻辑回归 nomogram Logistic Regression Nomogram
...
# 生成逻辑回归 nomogram PDF
# Generate logistic regression nomogram PDF
pdf("nomogram_regplot_glm.pdf", width = 10, height = 8)
regplot(pbcglm,
        observation=pbc[1,],
        points = TRUE,
        subticks = TRUE,
...
![](nomogram_regplot_glm.pdf)

## 绘制calibration curve进行验证 Plot calibration curve for validation
...
```{r}
set.seed(123)  # 设置随机种子以确保结果可重复 / Set seed for reproducibility
...
# 手动绘制 calibration 曲线，避免默认连线
# Manually draw calibration curve to avoid default connecting lines
plot(cal8[,c('mean.predicted',"KM")],
     type = 'b',
     pch = 16,
     lwd = 2,
     col = c("#B2182B"),
...
# 添加误差棒 - 使用 Hmisc::errbar 的正确参数格式
errbar(x = cal8[,1], y = cal8[,2], yplus = cal8[,4], yminus = cal8[,3],
        col = c("#B2182B"), lwd = 2, lty = 1)
box(lwd = 1)
abline(0,1,lty= 3, lwd = 2, col =c("#224444"))
```

### 同时展示两条curve Show two curves at the same time
...
修改 `regplot` 包的源代码，使其能够在 R Markdown 渲染时自动生成 PDF 文件。

This module has modified the source code of the `regplot` package to enable automatic PDF generation during R Markdown rendering.
